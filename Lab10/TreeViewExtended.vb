Public Class TableView
    ...
    ''' </summary>
    ''' <remarks>To have column auto-sizing ensure to add TableView to the parent after institation.</remarks>
    Sub New()
    ...
    Public Sub HighlightColumn(ColumnColor As Color, ColumnIndex As Integer)
    ...
    Public Sub HighlightRow(RowColor As Color, RowIndex As Integer)
    ...
    ''' <param name="Rowindex">Integer</param>
    ''' <param name="ColumnIndex">Integer</param>
    Public Sub HighlightCell(CellColor As Color, Rowindex As Integer, ColumnIndex As Integer)
    ...
    Public Sub HighlightAlernatingRows(RowColor1 As Color, RowColor2 As Color)
        Dim booleanAlternate As Boolean = True
        Dim index As Integer = 0
        For Each row As ListViewItem In Me.Items
            If booleanAlternate Then
                Me.HighlightRow(RowColor1, index)
                booleanAlternate = False
            Else
                Me.HighlightRow(RowColor2, index)
    ...
    Public Sub HighlightAlernatingColumns(ColumnColor1 As Color, ColumnColor2 As Color)
        Dim booleanAlternate As Boolean = True
        Dim index As Integer = 0
        For Each row As ListViewItem.ListViewSubItem In Me.Items(0).SubItems
            If booleanAlternate Then
                Me.HighlightColumn(ColumnColor1, index)
                booleanAlternate = False
            Else
                Me.HighlightColumn(ColumnColor2, index)
                booleanAlternate = True
            End If
            index = index + 1
        Next
    ...
    Public Sub OnTableViewColumnWidthChanging(send As Object, e As ColumnWidthChangingEventArgs)
